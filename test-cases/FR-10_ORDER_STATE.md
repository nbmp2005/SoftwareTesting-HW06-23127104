...
| Execution | Latest Tier-A Newman evidence reconciled: `0 PASS`, `48 FAIL`, `4 BLOCKED`; mapping `52/52` TC IDs |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `FR10-AI-001` | `AI` | FR-10; FR-18; API §6.2 | ST, self-transition | P1 | `ADM-A`; `O-P=pending` | `PUT /api/admin/orders/O-P/status`; target `pending`; required headers | `409` per ASM-03; JSON error; `O-P` remains `pending`, `O-U` unchanged | Restore `O-P/O-U` snapshots |  |  |  | `FAIL` |
| `FR10-AI-002` | `AI` | FR-10; FR-18; API §6.2 | ST, valid edge | P0 | `ADM-A`; `O-P=pending` | Same admin API; target `confirmed` | `200` per ASM-02; body `id=O-P,status=confirmed`, no sensitive fields; only `O-P→confirmed` | Restore `O-P/O-U` snapshots |  |  |  | `FAIL` |
| `FR10-AI-003` | `AI` | FR-10; FR-18; API §6.2 | ST, skip | P0 | `ADM-A`; `O-P=pending` | Admin API; target `shipping` | `409` per ASM-03; error; `O-P` stays `pending`, `O-U` unchanged | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-004` | `AI` | FR-10; FR-18; API §6.2 | ST, skip | P0 | `ADM-A`; `O-P=pending` | Admin API; target `delivered` | `409` per ASM-03; error; no state/version change | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-005` | `AI` | FR-10; FR-18; API §6.2 | ST, valid cancel | P0 | `ADM-A`; `O-P=pending` | Admin API; target `canceled` | `200` per ASM-02; body identifies `O-P,canceled`; only `O-P→canceled` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-006` | `AI` | FR-10; FR-18; API §6.2 | ST, backward | P0 | `ADM-A`; `O-CF=confirmed` | Admin API; target `pending` | `409`; error; `O-CF` remains `confirmed`, sentinel unchanged | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-007` | `AI` | FR-10; FR-18; API §6.2 | ST, self-transition | P1 | `ADM-A`; `O-CF=confirmed` | Admin API; target `confirmed` | `409`; error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-008` | `AI` | FR-10; FR-18; API §6.2 | ST, valid edge | P0 | `ADM-A`; `O-CF=confirmed` | Admin API; target `shipping` | `200`; body `id=O-CF,status=shipping`; only `O-CF→shipping` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-009` | `AI` | FR-10; FR-18; API §6.2 | ST, skip | P0 | `ADM-A`; `O-CF=confirmed` | Admin API; target `delivered` | `409`; error; `O-CF` stays `confirmed` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-010` | `AI` | FR-10; FR-18; API §6.2 | ST, valid cancel | P0 | `ADM-A`; `O-CF=confirmed` | Admin API; target `canceled` | `200`; body `id=O-CF,status=canceled`; only target changes | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-011` | `AI` | FR-10; FR-18; API §6.2 | ST, backward | P0 | `ADM-A`; `O-S=shipping` | Admin API; target `pending` | `409`; error; `O-S` remains `shipping` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-012` | `AI` | FR-10; FR-18; API §6.2 | ST, backward | P0 | `ADM-A`; `O-S=shipping` | Admin API; target `confirmed` | `409`; error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-013` | `AI` | FR-10; FR-18; API §6.2 | ST, self-transition | P1 | `ADM-A`; `O-S=shipping` | Admin API; target `shipping` | `409`; error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-014` | `AI` | FR-10; FR-18; API §6.2 | ST, valid edge | P0 | `ADM-A`; `O-S=shipping` | Admin API; target `delivered` | `200`; body `id=O-S,status=delivered`; only target changes | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-015` | `AI` | FR-10; FR-18; API §6.2 | ST, forbidden cancel | P0 | `ADM-A`; `O-S=shipping` | Admin API; target `canceled` | `409`; error; `O-S` remains `shipping` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-016` | `AI` | FR-10; FR-18; API §6.2 | ST, final state | P0 | `ADM-A`; `O-D=delivered` | Admin API; target `pending` | `409`; error; `O-D` remains `delivered` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-017` | `AI` | FR-10; FR-18; API §6.2 | ST, final state | P0 | `ADM-A`; `O-D=delivered` | Admin API; target `confirmed` | `409`; error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-018` | `AI` | FR-10; FR-18; API §6.2 | ST, final state | P0 | `ADM-A`; `O-D=delivered` | Admin API; target `shipping` | `409`; error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-019` | `AI` | FR-10; FR-18; API §6.2 | ST, final self | P1 | `ADM-A`; `O-D=delivered` | Admin API; target `delivered` | `409`; error; no new event/version or mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-020` | `AI` | FR-10; FR-18; API §6.2 | ST, final state | P0 | `ADM-A`; `O-D=delivered` | Admin API; target `canceled` | `409`; error; remains `delivered` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-021` | `AI` | FR-10; FR-18; API §6.2 | ST, final state | P0 | `ADM-A`; `O-X=canceled` | Admin API; target `pending` | `409`; error; `O-X` remains `canceled` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-022` | `AI` | FR-10; FR-18; API §6.2 | ST, final state | P0 | `ADM-A`; `O-X=canceled` | Admin API; target `confirmed` | `409`; error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-023` | `AI` | FR-10; FR-18; API §6.2 | ST, final state | P0 | `ADM-A`; `O-X=canceled` | Admin API; target `shipping` | `409`; error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-024` | `AI` | FR-10; FR-18; API §6.2 | ST, final state | P0 | `ADM-A`; `O-X=canceled` | Admin API; target `delivered` | `409`; error; remains `canceled` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-025` | `AI` | FR-10; FR-18; API §6.2 | ST, final self | P1 | `ADM-A`; `O-X=canceled` | Admin API; target `canceled` | `409`; error; no duplicate cancellation event/version | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-026` | `AI` | FR-10; API §4.6 | ST, valid cancel | P0 | `USR-A` owns `O-P=pending` | `PUT /api/orders/O-P/cancel`; user token + student header | `200` per ASM-01/02; body `O-P,canceled`; only owned order changes | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-027` | `AI` | FR-10; API §4.6 | ST, valid cancel | P0 | `USR-A` owns `O-CF=confirmed` | User cancel API for `O-CF` | `200`; body `O-CF,canceled`; only target changes | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-028` | `AI` | FR-10; API §4.6 | ST, forbidden cancel | P0 | `USR-A` owns `O-S=shipping` | User cancel API for `O-S` | `409` per ASM-01/03; error; remains `shipping` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-029` | `AI` | FR-10; API §4.6 | ST, final state | P0 | `USR-A` owns `O-D=delivered` | User cancel API for `O-D` | `409`; error; remains `delivered` | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-030` | `AI` | FR-10; API §4.6 | ST, final state | P0 | `USR-A` owns `O-X=canceled` | User cancel API for `O-X` | `409`; error; no duplicate cancellation side effect | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-031` | `AI` | FR-10; SEC-02; API §4.6 | Security, IDOR | P0 | `USR-B` does not own `O-P=pending` | User cancel API for `O-P` using `USR-B` token | `403` per ASM-05; generic error without order data; `O-P` unchanged | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-032` | `AI` | FR-10; SEC-02; API §4.6 | Security, IDOR | P0 | `USR-B` does not own `O-CF=confirmed` | User cancel API for `O-CF` using `USR-B` token | `403`; non-enumerating error; `O-CF` unchanged | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-033` | `AI` | FR-10; SEC-02; API §4.6 | Security, auth | P0 | No authenticated actor; `O-P=pending` | User cancel API with no `Authorization` header | `401` per ASM-05; generic JSON error; order unchanged | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-034` | `AI` | FR-10; SEC-02; API §4.6 | Security, auth | P0 | Invalid/expired fixture token; `O-P=pending` | User cancel API with `Bearer invalid.fixture.token` | `401`; no order details; state unchanged | Restore snapshots |  |  |  | `FAIL` |
...
| `FR10-AI-036` | `AI` | FR-10; FR-12; SEC-02/03; API §6.2 | Security, auth | P0 | No token; `O-P=pending` | Admin API target `confirmed`, no Authorization | `401`; generic error; no mutation | Restore snapshots |  |  |  | `FAIL` |
...
| `FR10-AI-041` | `AI` | FR-10; FR-18; API §6.2 | EP, required field | P0 | `ADM-A`; `O-P=pending` | Admin API with body `{}` | `400` per ASM-04; validation error names no sensitive data; state unchanged | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-042` | `AI` | FR-10; FR-18; API §6.2 | EP, null | P1 | `ADM-A`; `O-P=pending` | Admin API body `{"status":null}` | `400`; schema/validation error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-043` | `AI` | FR-10; FR-18; API §6.2 | EP, enum | P0 | `ADM-A`; `O-P=pending` | Admin API body `{"status":"refunded"}` | `400`; allowed-domain validation error; no mutation | Restore snapshots |  |  |  | `FAIL` |
| `FR10-AI-044` | `AI` | FR-10; ASM-FR10-06 | Concurrency, atomicity | P0 | `ADM-A` and owning `USR-A`; one `O-P=pending` | Synchronize admin `pending→confirmed` and user cancel requests | Responses/state match a legal serial order per ASM-06: cancel-first gives one commit; confirm-first may give two legal commits; final `canceled`, no lost/hybrid/duplicate mutation, sentinel unchanged | Dedicated order, barrier, DB snapshot restore |  |  |  | `FAIL` |
...
| `FR10-AI-047` | `AI` | FR-10; SEC-05; API §6.2 | Security, injection | P0 | `ADM-A`; `O-P=pending` | Admin API body `{"status":"confirmed' OR '1'='1"}` | `400` per ASM-04; no SQL/internal detail; no state or multi-row mutation; behavioral probe only | Restore/compare target and sentinel |  |  |  | `FAIL` |
...
| SHA-256 | `10CD5E0F2FB9789095AB4F4C6258A9B42292E29BC16126E0E722B2082BEC1D89` |
| Run window / host | `2026-09-02T15:04:34.504Z`–`2026-09-02T15:04:42.887Z`; `http://localhost:3000` |
| Raw run counts | 52 iterations; 110 request executions; 481 assertions; 88 failed assertions |
| Mapping coverage | 52/52 unique design TC IDs; 0 orphan executions |
| TC outcomes | 0 PASS; 48 FAIL; 4 BLOCKED; executed under repository convention = 48 |
...
- FAIL: the remaining 48 mapped IDs. These are assertion-level oracle violations, not 48 separate product bugs. Only the invalid-transition group is confirmed as `BUG-002`. `BUG-003` and `BUG-004` remain triage-pending because dirty shared state/duplicate user identity prevent their intended role, ownership and race oracles; other patterns include `409` versus `400` ambiguity and response-schema/test-script mismatch.

## 11. Quality-gate result

- Structural count: 47 unique AI IDs plus 5 unique HUMAN IDs; all 52 rows map to the authoritative run, with 0 PASS, 48 FAIL and 4 BLOCKED.
...
- Remaining gaps: nine contract decisions (`ASM-FR10-01`–`09`), deterministic reset per iteration, distinct user identities, repair/rerun of four blocked verification fixtures, clean reruns for `BUG-003`/`004`, and human audit of the 47 AI candidates.

**Generation status: COMPLETE; execution reconciliation: COMPLETE for this artifact.** Human audit, fixture-isolated reruns and complete failure triage remain incomplete.
